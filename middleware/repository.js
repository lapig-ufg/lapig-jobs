...
module.exports = function(app) {
	...
	Repository.init = function (callback) {
		Repository.client.connect((err, client) => {
			...
			Repository.dbJobs = client.db(config.mongo.dbJobs);
			...
			Repository.dbJobs.listCollections().toArray((err, collection) => {
				if (err) {
					return callback(err);
				}
				const forEachOne = function (collection, callback) {
					const name = collection.name.substr(collection.name.indexOf('\.') + 1);
					if(name !== 'indexes') {
						Repository.dbJobs.collection(name, function(err, repository) {
							if(err){
								console.log(err)
							}
							Repository.collectionsJobs[name] = repository;
							callback();
						});
					} else {
						callback();
