 node {
    ...
    def application_name= "app_lapig_jobs"
        ...
        stage('Checkout') {
            git branch: 'main',
            url: 'https://github.com/lapig-ufg/lapig-jobs.git'
        }
        stage('Validate') {
        ...
        stage('Deploy container on PROD') {
                        ...
                        configFileProvider([configFile(fileId: "$File_Json_Id_APP_LAPIG_JOBS_PROD", targetLocation: 'container-lapig-jobs-deploy-prod.json')]) {
